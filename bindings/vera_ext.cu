// Vera Python bindings (nanobind).
//
// Compiled as CUDA (.cu) rather than plain C++ because the renderer's public
// headers are .cuh and carry __global__ / __device__ qualifiers that only nvcc
// parses. This TU itself contains no kernels — it just drives the host-side
// entry points (UploadScene, BuildSceneLighting, RenderHWSS) and marshals the
// resolved framebuffer into a NumPy array.
//
// v1 surface: a Scene you build imperatively, material/env factories, a Camera,
// and render() -> (H, W, 3) float32 of the tonemapped image. Deliberately a thin
// 1:1 wrapper over what src/Main.cu already does; grow from here.
