...
// Local print-and-abort error checking (no sutil/Exception.h - it pulls in GL/GLFW/imgui).
...
namespace Vera::Core {
	// No per-record data - everything lives in OptixTraversalParams, not the SBT.
	...
	OptixTraversalContext InitOptixTraversal(const GeometryBuffers& geom) {
		...
		// -- GAS from geom's flat triangle soup. NXB::Triangle is {float3 v0,v1,v2} with no
		// padding, so OptiX reads it directly as OPTIX_VERTEX_FORMAT_FLOAT3. --
		...
		// -- IAS: one instance wrapping the GAS with the scene's instance transform.
		// Single-instance only - a multi-mesh scene would need one GAS per distinct mesh. --
		{
			Instance hInst{};
			CUDA_CHECK(cudaMemcpy(&hInst, geom.m_DevInstances, sizeof(Instance), cudaMemcpyDeviceToHost));

			OptixInstance optixInst = {};
			// Instance::m_Transform is 3 float4 rows == OptixInstance::transform's 3x4 row-major layout
