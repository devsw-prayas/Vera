...
int main() {
	...
	if (rgb2spec.res == 0) {
		fprintf(stderr, "Failed to load RGB2Spec table\n");
		return 1;
	}

	std::vector<Material> hMaterials;
	hMaterials.push_back(MakeLambertian(rgb2spec, make_float3(0.035f, 0.045f, 0.06f))); // 0: floor
	hMaterials.push_back(MakeEmissive(rgb2spec, make_float3(0.72f, 0.86f, 1.f), 7.f));  // 1: back wall
	// 2: diamond, Sellmeier n@587.6nm ~2.417 (Peter 1923)
	...
	hMaterials.push_back(MakeEmissive(rgb2spec, make_float3(1.f, 0.92f, 0.78f), 3.f)); // 3: ceiling fill
	...
	if (hdri.ok()) {
		...
		envMap = MakeImageEnvMap(rgb2spec, src, 1.f);
	} else {
		printf("Env: '%s' not found, using black\n", hdriPath);
	...
	RenderHWSS(scene.geom, d_materials, lighting.d_media, lighting.lightBvh, envMap,
			   camera, fb, d_outRGB, /*spp*/ 1024, /*maxBounces*/ 32,
